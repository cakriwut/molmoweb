...
WEBOLMO_UBER_SIMPLE_TEMPLATE = Template(
"""
# GOAL
{{ task_description }}
...
WEBOLMO_SIMPLE_TEMPLATE = Template(
"""
# GOAL
{{ task_description }}
...
# Used for human_trajs, v0, and node_traversal
WEBOLMO_BASE_TEMPLATE = Template(
"""
# GOAL
{{ task_description }}
...
WEBOLMO_UBER_SIMPLE_TEMPLATE_MINI = Template(
"""
# GOAL
{{ task_description }}
...
WEBOLMO_SIMPLE_TEMPLATE_MINI = Template(
"""
# GOAL
{{ task_description }}
...
WEBOLMO_BASE_TEMPLATE_MINI = Template(
"""
# GOAL
{{ task_description }}
...
MOLMOWEB_BASE_TEMPLATE = Template(
"""
# GOAL
{{ task_description }}
...
def apply_keyword_prompt(prompts, example, rng, keywords=None, dbg=False):
    if isinstance(prompts, list):
        ...
        assert len(keywords) == len(
            set(keywords)
        ), f"Repeated keywords in {keywords}"
        assert all(
            keywords == x for x in all_keywords
        ), f"Inconsistent keywords in prompts {all_keywords}"
        assert not any(
            "{" not in word[1:-1] and "}" in word[1:-1] for word in keywords
        )
...
class DataFormatter(BaseConfig):
    ...
    default_inference_len: Optional[
        int
    ] = 65  # Inference len for length-conditioned prompting
    select_answer: str = (
        "best"  # How to select answer for questions with many answers
    )
    ...
    def get_user_prompt(self, example, is_training=True, for_inference=False, rng=None):
        ...
        if "prompt" in example:
            ...
            elif style in ["web_grounding", "screenshot_qa"]:
                # prompt = WEBOLMO_UBER_SIMPLE_TEMPLATE.render(example)
                prompt = WEBOLMO_UBER_SIMPLE_TEMPLATE_MINI.render(example)
                output = example['answer']
                metadata = example.get('metadata', {})

            elif style == "molmo_web_think":
                prompt = MOLMOWEB_THINK_TEMPLATE.render(example)
                output = example['answer']
                metadata = example.get('metadata', {})
            elif style == "molmo_web_base":
                prompt = MOLMOWEB_BASE_TEMPLATE.render(example)
                output = example['answer']
                metadata = example.get('metadata', {})
    ...
    def _format_example(
        ...
        if (
            self.image_last
            and ("image" in example or "video" in example)
            and tokenizer.IMAGE_PROMPT not in messages[0]
        ):
    ...
    def __call__(
        self, ex: Dict, is_training, for_inference, rng
    ) -> Tuple[Dict, Dict]:
        ...
        if "message_list" in ex:
            if self.p_one_message and rng.random() < self.p_one_message:
                ...
                for ix, msg_list in enumerate(ex["message_list"], start=1):
                    msg_list[
                        "question"
                    ] = f"{' ' if ix != 0 else ''}Q{ix}: {msg_list['question']}"
        ...
        if "message_list" in ex:
            ...
            return [
                self._format_example(msg, ex, is_training, for_inference, rng)[
                    0
                ]
